...
async def webapp_order(message: types.Message):
    ...
    await message.answer(
        "✅ Заказ принят! Спасибо, что выбрали FURKATOVICH — Mazali Taomlar 😊",
    ...
    if isinstance(items_list, list) and items_list:
        for it in items_list:
            name = it.get("name_lang") or it.get("name_ru") or it.get("name") or it.get("id") or "—"
            qty = it.get("qty") or 0
            price = it.get("price") or it.get("p") or ""
            if price:
                items_lines.append(f"• {safe_html(name)} × <b>{safe_html(qty)}</b> — {safe_html(price)} сум")
            else:
    ...
    phone = data.get("phone") or customer.get("phone") or data.get("phoneInput") or ""
    address = data.get("address") or customer.get("address") or data.get("addr") or data.get("addrInput") or ""
    payment_raw = get_first(data, ["payment", "pay", "paymentMethod", "payment_method", "payMethod", "pay_method", "payment_type", "pay_type"], "")
    type_raw = get_first(data, ["type", "order_type", "orderType", "fulfillment", "fulfillmentType", "deliveryType", "delivery_type", "receiveType", "receive_type"], "")
    ...
    admin_text = (
        "📩 <b>НОВЫЙ ЗАКАЗ — FURKATOVICH | Mazali Taomlar</b>\n\n"
